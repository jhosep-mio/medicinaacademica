...
import { type ImagenState, type bannersValuesModificate } from '../../../shared/Interfaces'
import { ImageUpdate } from '../../../shared/ImageUpdate'

export const EditarMarca = (): JSX.Element => {
  const { id } = useParams()
  ...
  const saveCategoria = async (values: bannersValuesModificate): Promise<void> => {
    ...
    try {
      const respuesta = await axios.post(`${Global.url}/updateMarca/${id ?? ''}`, data, {
        headers: {
  ...
  const getBanner = async (): Promise<void> => {
    setLoadingComponents(true)
    const request = await axios.get(`${Global.url}/oneMarca/${id ?? ''}`, {
      headers: {
        Authorization: `Bearer ${
