...
import { SchemaCompra } from '../../shared/Schemas'
...
export const PersonaNatural = ({
    ...
    if (request.data.existe && !auth.id) {
      ...
      if (nombresCursosComprados.length > 0) {
        ...
        const listaHtmlCursos = nombresCursosComprados
          .map(nombreCurso => `<li style="margin-top: 10px; font-weight: 700;">- ${nombreCurso}</li>`)
          .join('')
  ...
  const validarCursosComprados = (cursosCarrito: carrito[]): string[] => {
    const cursosYaComprados = cursosCarrito.filter(cursoCarrito => {
      return cursos.some((cursoComprado: cursosCompradosValues) => {
        ...
        const cursosCompradosIds = productosComprados.map((producto: any) => String(producto.id))
