          ...
          const enviarDatos = async (): Promise<void> => {
            ...
            try {
              const respuesta = await axios.post(
                `${Global.url}/saveComentariofinal/${cursoId ?? ''}`,
                data,
                {
                  headers: {
    ...
    >
      <DialogContent>
        <section className="w-full h-full">
          <div className="bg-[#121f3d] w-full h-full mx-auto border-2 border-[#24385b] p-6 relative flex items-center justify-center flex-col">
            <span className="absolute top-4 right-12 text-4xl text-secondary-70 cursor-pointer" onClick={() => { setOpenComentario(false) }}>
              x
            </span>
              ...
              <div className="w-full rounded-xl bg-primary border border-secondary-70 p-8 text-2xl text-gray-300 flex items-center gap-4">
                <div className="cursor-pointer flex items-start w-full max-h-[200px] overflow-y-auto">
                  <textarea
                    ...
                    className="w-full flex-1 h-full pl-4 outline-none py-4 resize-none overflow-hidden text-2xl"
                  ></textarea>
                </div>
              </div>
            ...
            <div className="flex gap-10 justify-end">
              <button className="text-[1.6rem] text-secondary-70 " onClick={() => { setOpenComentario(false) }}>
                Cancelar
              </button>
              {!loading
                ? (
